...
from multistage_validation.functional_coating.benchmark import CONTROL_AXES, full_line_grid, individual_best_chain
...
def create_oracle_bundles(bundle_root: Path) -> None:
    ...
    base = "multistage_validation.functional_coating"
    for stage in STAGES:
        manifest = _read_json(ROOT / "multistage_validation" / "functional_coating" / stage / "manifest.json")
...
def trial_config(bundle_root: Path) -> dict[str, object]:
    process = _read_json(ROOT / "multistage_validation" / "functional_coating" / "process_manifest.json")
    margin_scales = {
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("usage: python -m multistage_validation.r001_validation TRIAL OUTPUT.json")

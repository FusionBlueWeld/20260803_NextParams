...
from multistage_validation.learned_r001_validation import ROOT, V004, BASE, _current_contexts, _lhs
...
def _evaluate(stage: str, inputs: dict[str, object]) -> dict[str, np.ndarray]:
    module = __import__(f"multistage_validation.functional_coating.{stage}.physics_model", fromlist=["evaluate_model"])
...
def consolidate_final_report(base_report: Path, dense_curing_reports: list[Path], output: Path) -> dict[str, object]:
    """128点の診断結果と、硬化512点で合格した最終結果を一つにまとめます。"""
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("usage: python -m multistage_validation.stage_closed_loop_validation PREFIX OUTPUT.json")

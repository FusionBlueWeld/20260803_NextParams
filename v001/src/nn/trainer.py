...
def train_network(
    ...
    for epoch in range(1, NN_MAX_EPOCHS + 1):
        ...
        gradients["w_skip"] = cache["inputs"].T @ gradient_output
        gradients["b_skip"] = np.sum(gradient_output, axis=0)
        gradients["w_out"] = cache["residual"].T @ gradient_output
        ...
        beta_2 = 0.999
        for name in weights:
            first_moment[name] = beta_1 * first_moment[name] + (1.0 - beta_1) * gradients[name]
            second_moment[name] = beta_2 * second_moment[name] + (1.0 - beta_2) * np.square(gradients[name])
    ...
    return NeuralTrainingResult(model=model, epochs=completed_epochs, normalized_mse=best_loss)

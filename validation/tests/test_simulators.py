...
from validation.simulators import Simulator, Variable, available, load, measure, read_csv, write_json
...
class SimulatorContracts(unittest.TestCase):
    ...
    def test_old_laser_grid_and_import_remain_equivalent(self):
        from validation.laser_welding_pseudo_experiment import (
...
class HarnessTests(unittest.TestCase):
    def test_response_space_uses_numeric_latest_and_rejects_stale(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch.object(benchmark, "read_csv", return_value=[{"data_rows": "26"}]) as reader:
                result = benchmark.response_metrics(load("milling"), trial, np.zeros((27, 3)), {})
    ...
    def test_minimize_maximize_and_empty_feasible(self):
        ...
        self.assertAlmostEqual(benchmark.regret(minimum, 3, 1, 10), .2)
        ...
        self.assertAlmostEqual(benchmark.regret(maximum, 8, 10, 10), .2)
        self.assertIsNone(benchmark.regret(maximum, None, 10, 10))

...
from validation.multistage.functional_coating.pipeline import DEFAULT_MATERIAL_STATE, LINE_INPUT_BOUNDS, evaluate_line
from validation.multistage.functional_coating.robustness import DEFAULT_STANDARD_DEVIATIONS, sample_conditions
...
def run(trial_name: str, report_path: Path, samples: int = 4096, seed: int = 20260909, exhaustive_samples: int = 1024) -> dict[str, object]:
    """既存trialの入力ばらつき評価をoracleの反復標本と比較して保存します。"""
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit("usage: python -m validation.multistage.src.checks.probabilistic_r001_validation TRIAL REPORT.json")

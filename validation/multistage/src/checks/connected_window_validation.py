"""連結窓の一変数断面を、決定論的な物理oracleと照合します。

中心の合否だけでなく、離れた許容区間も含めて誤許容と回収率を採点します。"""
...
REPORT = ROOT / "validation/multistage/results/connected_window_validation.json"
...
def _oracle_process_feasible(conditions: dict[str, float]) -> tuple[bool, dict[str, float | bool | list[str]]]:
    """接続範囲・工程内制約・最終仕様をすべて満たすか、物理モデルで判定します。"""
...
def _acceptance(center_ok, rows, confusion, empty_windows, unsafe_overshoots, scan_points, expected_axes,
    ...
    """全軸評価・中心合格・誤許容なし・回収率などをすべて要求して合否を返します。"""
...
def run() -> dict[str, object]:
    """固定trialの保存済み連結窓を読み、oracleで採点した結果をREPORTへ保存します。"""

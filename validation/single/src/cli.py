"""単工程検証のコマンドを受け取り、担当する処理へ振り分けます。

run.pyから呼ばれる入口です。物理式は工程フォルダ、反復実験はbenchmark.py、
比較表の保存はreporting.pyへ委譲します。"""
...
from validation.single.src import benchmark
from .settings import VALIDATION_ROOT as ROOT, PROBLEM_HEADER
from .simulators import available, load
from .data_loader import write_csv, write_json
from .reporting import suite_report
...
def parser() -> argparse.ArgumentParser:
    ...
    for cmd in (run, suite):
        ...
        cmd.add_argument("--output", type=Path, help="新規の結果フォルダ（省略時はvalidation/single/results/multiphysics配下）")
    return result

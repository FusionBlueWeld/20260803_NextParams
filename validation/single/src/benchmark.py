"""実際のoptimizer CLIを呼び、推薦と仮想実験を繰り返します。

初期測定 → 推薦 → 正解モデルで検査 → 測定CSVへ追記、の順で進めます。
真値とノイズ付き測定値は別々に保存し、採点はmetrics.pyへ委譲します。"""
...
from .data_loader import read_csv, write_csv, write_json
from .settings import VALIDATION_ROOT as ROOT, PROJECT_ROOT, PROBLEM_HEADER
from .simulators import Simulator, measure
from .metrics import regression_metrics, regret, summarize_best, response_metrics
...
def source_hashes(folder: Path) -> dict[str, str]:
    """改行をLFにそろえ、フォルダ内のPythonコードのハッシュを求めます。"""
...
def audit(sim: Simulator) -> dict:
    """全候補と初期点を評価し、適合件数・最良値・出力範囲をまとめます。"""
...
def validate_recommendations(sim: Simulator, points: np.ndarray, seen: set) -> None:
    """推薦が領域内・グリッド上にあり、既測定点や同一バッチと重複しないか確認します。"""
...
def run(sim: Simulator, version: str, trial_name: str, result_dir: Path,
    ...
    """1工程・1版の検証を実行し、準備中の失敗も新規結果フォルダへ記録します。"""
...
def _run(sim: Simulator, version: str, trial_name: str, result_dir: Path,
        iterations: int = 15, recommendations: int = 1, seed: int = 0, noise: float = 0.0) -> dict:
    """初期点の準備、閉ループ実験、最終予測、採点の順に1試行を進めます。"""
    ...
    # 1. 正解モデルと初期点を用意します。全候補の真値は採点側にだけ保持します。
    ...
    # 2. 実行条件と初期測定を保存し、途中失敗でも再現に必要な情報を残します。
    ...
    metadata = {"simulator": sim.id, "optimizer": version, "trial": trial_name,
                ...
                # 分割した採点・設定・読書きも含め、実行器全体を追跡します。
                "runner_sha256_lf": {
                    **{f"src/{name}": digest for name, digest in source_hashes(ROOT / "src").items()},
                    **{p.name: hashlib.sha256(p.read_bytes().replace(b"\r\n", b"\n")).hexdigest()
                       for p in [ROOT / "run.py", PROJECT_ROOT / "main.py"]},
                },
    ...
    try:
        ...
        write_csv(result_dir / "truth.csv", sim.experiment_header, truth_rows)
        # 3. 推薦 → 真値評価 → 測定ノイズ → CSV追記の順序を各反復で守ります。

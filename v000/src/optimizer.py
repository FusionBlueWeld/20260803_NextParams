...
このファイルが、v000のベイズ最適化の中心です。処理を小さな関数に分け、
...
from .acquisition import constraint_probability, expected_improvement
...
def make_candidates(problem: ProblemDefinition) -> np.ndarray:
    ...
    # itertools.productは「各軸から1つずつ選ぶ」全組み合わせを作ります。
...
def remove_measured_candidates(
    ...
    measured_keys = {tuple(np.round(row, 12)) for row in measured_parameters}
    keep_rows = [tuple(np.round(row, 12)) not in measured_keys for row in candidates]
...
) -> dict[str, GaussianProcess]:
    """目的・制約・参考出力ごとにGaussian Processを作ります。"""

    models: dict[str, GaussianProcess] = {}
    for result in problem.result_variables:
        models[result.column] = GaussianProcess.fit(
...
def predict_in_chunks(
    ...
    """候補数が多くてもメモリを使いすぎないよう、分割して予測します。"""
    ...
    for start in range(0, len(candidates), chunk_size):
        ...
        mean, std = model.predict(candidates[start:stop])
        means.append(mean)
...
def run_optimization(
    ...
    raw_candidates = remove_measured_candidates(raw_candidates, prepared.raw_parameters)
    ...
    for result in problem.result_variables:
        mean, std = predict_in_chunks(models[result.column], normalized_candidates)
        predictions[result.column] = {"mean": mean, "std": std}
    ...
    feasible_observed = observed_feasible_mask(prepared, problem)
    ...
    diversity_state = calculate_diversity_state(
        ...
        objective_scale=models[problem.objective.column].y_scale,
